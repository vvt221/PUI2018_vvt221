...
for vehicle in range(no_active_buses):
    ...
    onward_calls = len(mta_vehicle_details[vehicle]['MonitoredVehicleJourney']['OnwardCalls'])
    ...
    if bool(mta_vehicle_details[vehicle]['MonitoredVehicleJourney']['OnwardCalls']):
        
        stop_name = str(mta_vehicle_details[vehicle]['MonitoredVehicleJourney']['OnwardCalls']['OnwardCall'][0]['StopPointName'])
        
    
        stop_status = str(mta_vehicle_details[vehicle]['MonitoredVehicleJourney']['OnwardCalls']['OnwardCall'][0]['Extensions']['Distances']['PresentableDistance'])
